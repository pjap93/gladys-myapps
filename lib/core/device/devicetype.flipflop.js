...
function flipflop(param) {
  ...
  return gladys.utils.sql(queries.getDeviceType, [param.devicetype])
	...
	//console.log("params",param);
      ...
      if(param.value === types[0].min){
		param.value = types[0].max;
      } else {
		param.value = types[0].min;
      }
      ...
      if(types[0].machine && types[0].machine.length) {
        ...
        gladys.emit('devicetype-flipflop', data);
